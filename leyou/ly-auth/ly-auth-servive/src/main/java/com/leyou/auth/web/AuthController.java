...
public class AuthController {
    ...
    /**
     * 登录授权  向客户端返回一个token 一个携带用户信息的加密文件
     * @param username
     * @param password
     * @param response
     * @param request
    ...
    public ResponseEntity<Void> login(// 无需给前端浏览器返回token，token只有后端才需要使用，并且将token保存到cookie中
        ...
        CookieUtils.newBuilder(response).httpOnly().request(request).maxAge(cookieMaxAge * 60).build(cookieName, token);
    ...
    public ResponseEntity<UserInfo> verify(
        ...
        try {
            ...
            CookieUtils.newBuilder(response).httpOnly().request(request).maxAge(cookieMaxAge * 60).build(cookieName, newToken);
