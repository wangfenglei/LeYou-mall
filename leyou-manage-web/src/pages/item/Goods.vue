...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      editGoods(oldGoods) {
        // 发起请求，查询商品详情和skus
        //oldGoods.spuDetail =  this.$http.loadData("/item/spu/detail/" + oldGoods.id);
        //oldGoods.skus =  this.$http.loadData("/item/sku/list?id=" + oldGoods.id);
        ...
        this.$http.get("/item/sku/list?id="+ oldGoods.id )
          .then(({ data }) => {
            oldGoods.skus = data;
          });
        // 修改标记
